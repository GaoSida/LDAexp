...
int AliasSampler::sample(int doc, int token, int iteration)
{
    ...
    for (int i = 1; i <= corpus->K; i++)
    {
        ...
        sparse_value_list[i - 1] = n_td * (corpus->topic_by_word_cnt[k][word] + corpus->beta[word])
    ...
    double propOldSample = (corpus->topic_by_word_cnt[old_topic][word] + corpus->beta[word])
                         / (corpus->topic_token_sum[old_topic] + corpus->beta_sum);
    double propNewSample = (corpus->topic_by_word_cnt[new_topic][word] + corpus->beta[word])
